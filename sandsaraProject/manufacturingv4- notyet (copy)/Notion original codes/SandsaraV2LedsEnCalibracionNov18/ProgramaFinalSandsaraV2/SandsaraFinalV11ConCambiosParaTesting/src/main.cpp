///////abc flag for changes made.
////// Change time of interruption to test due to always turn on automaticallyt green, increase time to 1000 to 3000 ms (before just <3000)
////// TODO change spiral speed.
///////change current to 900
// Comente todo lo del sensor hall hya que se activa solo y encesito enviarlo hoy.

// tmcMotorA.ihold(16);
//	tmcMotorB.ihold(16);
//	tmcMotorA.irun(31);
//	tmcMotorB.irun(31);
// Value returned to 800
// tracksBeforeCalibration = 1; //abc
...
RTC_PCF8563 rtc;
DateTime now;
DateTime startTimeRTC; // Variable para almacenar hora de inicio
bool messageDisplayed = false;
#endif

bool resetHallPin = false;
...
void IRAM_ATTR manejarInterrupcion()
{
    ...
    bool estadoActual = digitalRead(SENSOR_HALL_PIN);

    if (estadoActual == LOW && ultimoEstado == HIGH)
    {
        tiempoInicio = millis();
    }
    else if (estadoActual == HIGH && ultimoEstado == LOW)
    {
        unsigned long duracion = millis() - tiempoInicio;
        /*
        if (duracion >= 3000 && duracion <= 10000) {
          botonPresionado = true;
        }
        */
        if (duracion >= 1000 && duracion < 3000)
        {
            Serial.println("Entrar a modo Test");
            // botonPresionado = true;
            // ledcAttachPin(BUZZER_PIN, 5);
            // cute._tone(NOTE_C7, 50, 10); // Genera el tono en el buzzer
            // 1tiempoInicio = millis();
            // #define internalTesting
            // #define calibIntermedia
        }
        else if (duracion >= 3000 && duracion <= 8000)
        {
            ...
            // ledcAttachPin(BUZZER_PIN, 5);
            //   cute._tone(NOTE_C7, 50, 10); // Genera el tono en el buzzer
            // cute._tone(NOTE_A7,50,30); // Genera el tono en el buzzer
            //  tiempoInicio = millis();
        }
    }
...
unsigned long startTimeSensorHall = 0;
//=================================================
...
void setup()
{
    ...
    if (dat_pin > 0 && dat_pin <= 682)
    {
        // Configuracion de Sandsara Original
        // #define originalSandsara
    }
    if (dat_pin > 682 && dat_pin <= 1364)
    {
        // Testing interno
        haloTest.Test();
    }
    if (dat_pin > 1364 && dat_pin <= 2046)
    {
        // Borrar la EEPROM
        for (int x = 0; x < 512; x++)
        {
    ...
    if (dat_pin > 2046 && dat_pin <= 2728)
    {
        // Configuracion libre
    }

    if (dat_pin > 2728 && dat_pin <= 3410)
    {
        // Configuracion libre
    }

    if (dat_pin > 3410 && dat_pin <= 4095)
    {
        // Configuracion de SandsaraV2
        // #define v2Sandsara
    }

    /*
    startTimeSensorHall = millis();

    while (millis() - startTimeSensorHall < 3000) {

        if (digitalRead(SENSOR_HALL_PIN) == LOW) {
          sensorActive = true;
          //break;
        }
        else
      ...
      if (sensorActive) {
        Serial.println("El sensor ha estado en estado bajo durante al menos 3 segundos.");
        #define internalTesting
        //#define calibIntermedia
        // Realiza alguna acción aquí si es necesario
      } else {
        Serial.println("El sensor no ha estado en estado bajo durante 3 segundos.");
        #define calibIntermedia
        // Continúa con el programa de manera normal
      }
      */
    // #define calibIntermedia
...
// changePalette(CODE_NOSD_PALLETE);
    ...
    */
    ...
    ledcSetup(5, 1000, 16); // channel, max frequency, resolution
#endif
    ...
    BluetoothSand.setModel(SANDSARAMODEL);
    ...
    // Test pines touch despues de bluetooth
    /*
...
//=========RTC============
#ifdef rtcInterno
        rtc.setTime(00, 00, 00, 17, 1, 2021); // 17th Jan 2021 00:00:00
#endif

#ifdef rtcExterno
        startTimeRTC = DateTime(2021, 1, 17, 0, 0, 0);
        rtc.adjust(startTimeRTC);
#endif
        //========================
    }
    else
...
#ifdef rtcExterno
        startTimeRTC = DateTime(2021, 1, 17, hourActualInt, minuteActualInt, 0);
        rtc.adjust(startTimeRTC);
#endif
    }
...
//====Select type of product====
    ...
    if (resetHallPin == true)
    {
        ...
        Serial.println("Se borro la Eeprom desde el sensor Hall");
    }
#endif
...
*/
    ...
    tmcMotorB.rms_current(800);

    Serial.println("Current set up");
    Serial.println(tmcMotorA.rms_current());
    Serial.println(tmcMotorB.rms_current());
    Serial.println(tmcMotorA.ihold());
    Serial.println(tmcMotorB.ihold());
    Serial.println(I_HOLD);
    // delay(2000000000000);
...
#endif
            Serial.print("On: ");
            Serial.print(hourOnInt);
            Serial.print(":");
            Serial.print(minuteOnInt);
            Serial.print(" ");
            Serial.print("Off: ");
            Serial.print(hourOffInt);
            Serial.print(":");
            Serial.print(minuteOffInt);
            Serial.println(" ");

            int actualTimeInMinutes = actualHour * 60 + actualMinute;
            int offTimeInMinutes = hourOffInt * 60 + minuteOffInt;
            int onTimeInMinutes = hourOnInt * 60 + minuteOnInt;

            if (actualTimeInMinutes >= offTimeInMinutes || actualTimeInMinutes < onTimeInMinutes)

            {

#ifdef rtcInterno
            unsigned long lastTriggerTime = 0; // Initialize the last trigger time
            unsigned long triggerInterval = 5000; // Set the trigger interval to 5 seconds (5000 milliseconds)

            pauseModeGlobal = true;
            Bluetooth::setStatus(MODE_PAUSE);
            ledsOffGlobal = true;

            while (pauseModeGlobal == true)
            {
                // Check if the current time is at least triggerInterval milliseconds after the last trigger time
                if (millis() - lastTriggerTime >= triggerInterval)
                {
                    pauseModeGlobal = true;
                    Bluetooth::setStatus(MODE_PAUSE);
                    Serial.println("Inside the onofftime Cycle. ");
                    bluetoothTimeOffOn = romGetBluetoothTimeOffOn();
                    std::vector<char> vec(bluetoothTimeOffOn.begin(), bluetoothTimeOffOn.end());
                    ...
                    actualTimeInMinutes = actualHour * 60 + actualMinute;
                    int onTimeInMinutes = hourOnInt * 60 + minuteOnInt;
                    int offTimeInMinutes = hourOffInt * 60 + minuteOffInt;
                     ...
                     Serial.print(hourOnInt);
                     Serial.print(":");
                     Serial.println(minuteOnInt);
                     Serial.print("Off time: ");
                        Serial.print(hourOffInt);
                        Serial.print(":");
                        Serial.println(minuteOffInt);
                        

                     if (actualTimeInMinutes >= onTimeInMinutes && actualTimeInMinutes  <= offTimeInMinutes)
                     {
                         Serial.print("Outside cycle");
                         Serial.print(actualTimeInMinutes);
                         Serial.print(":");
                         Serial.println(onTimeInMinutes);
                         suspensionModeGlobal = false;
                         pauseModeGlobal = false;
                         Bluetooth::setStatus(MODE_PLAY);
                    }
                    // Then update the last trigger time
...
int runFile(String fileName)
{
    ...
    } // Aqui se cierra el ciclo while que esta reproduciendo el archivo
    ...
    */
    ...
    else if (posisionCase == 0)
    {
        ...
        {
            bool resultError1 = false;
            bool resultError2 = false;
            Serial.println("Se calculara desfase de los brazos");
            resultErrorGap = haloCalib.checkErrorGap(resultError1, resultError2);
    ...
    if (posisionCase == 2)
    {
        ...
        if (intermediateCalibration == true || tracksPlayed >= tracksBeforeCalibration)
        {
            ...
            Serial.println("calibration starts");
            haloCalib.verificacion_cal(); // abc
            Serial.println("calibration ends");
    ...
    /*
       #ifdef internalTesting

       now = rtc.now();
       unsigned long elapsedMinutes = (now.unixtime() - startTimeRTC.unixtime()) / 60;
       //Serial.println("Minutos totales");
       //Serial.println(elapsedMinutes);
       if (!messageDisplayed && elapsedMinutes >= (480)) {

       messageDisplayed = true;  // Marca que el mensaje ya se mostró



               if(resultError1 == true || resultError2 == true)
               {
                   changePalette(CODE_NOSD_PALLETE);
                   while(true)
                   {
...
void ledsFunc(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        */
        if (botonPresionado)
        {
            Serial.println("¡Se activo el modo test desde sensor Hall!");
